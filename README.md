...
In this demo, an employee uses Gemini CLI to check their available vacation days, request time off, and update the request — all in natural language. They then approve the pending time-off requests from their team through the same conversational interface.
